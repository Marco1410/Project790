...
contains
  ...
  subroutine initMesh(cfdAppl)
    ...
    do i = 1, nPoint
       ...
       call cfdAppl%node(iPoint)%assignDof(1,cfdAppl%model%dof(iPoint*4-3))
       call cfdAppl%node(iPoint)%assignDof(2,cfdAppl%model%dof(iPoint*4-2))
       call cfdAppl%node(iPoint)%assignDof(3,cfdAppl%model%dof(iPoint*4-1))
       call cfdAppl%node(iPoint)%assignDof(4,cfdAppl%model%dof(iPoint*4))
